...
// ── Coordinate projection (Mercator matching ComposableMap settings) ──────────
...
export default function EuropeMap({ onRevealReady }: Props) {
  ...
  useEffect(() => {
    const t1 = setTimeout(() => setShowRoutes(true),  900)
    const t2 = setTimeout(() => setShowMarkers(true), 900 + routesTotalMs)
    const t3 = setTimeout(() => setBurst(true),       AUTO_TRANSITION_MS)
    return () => [t1, t2, t3].forEach(clearTimeout)
  }, [])

  useEffect(() => {
    if (burst) { const t = setTimeout(onRevealReady, 1100); return () => clearTimeout(t) }
  }, [burst, onRevealReady])

  // Transform origin as % of SVG for the zoom-out
    ...
    <div className="relative w-full min-h-screen flex flex-col items-center bg-void">
      ...
      <div className="relative flex-1 w-full max-w-5xl mx-auto px-3 pb-4 flex flex-col">
        ...
        >
          ...
          <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse 80% 80% at 50% 50%, #0C021A 0%, #06020E 100%)' }} />

          {/* Geographic map — zoom out from Linköping */}
          <motion.div
